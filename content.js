// content.js - Enhanced LinkedIn Profile Extractor with User Authentication
...
class LinkedInExtractor {
  ...
  // Get stored username from localStorage - Fixed method
  getStoredUsername() {
      try {
          // Try multiple ways to get the username
          let username = localStorage.getItem('linkedin_scraper_username');
          
          // If not found, try chrome storage API
          if (!username && typeof chrome !== 'undefined' && chrome.storage) {
              chrome.storage.local.get(['linkedin_scraper_username'], (result) => {
                  username = result.linkedin_scraper_username;
              });
          }
  ...
  // Enhanced duplicate check with username
  async checkDuplicate(username) {
      try {
          const response = await fetch(`https://linkedinscrap-e4dpdhcuc7fgd7fk.eastasia-01.azurewebsites.net/api/profiles?username=${encodeURIComponent(username)}`);
  ...
  // Enhanced main extraction method - FIXED AUTHENTICATION CHECK
  async extractProfile() {
      ...
      console.log('Authentication check - Username:', username);
      ...
      try {
          this.showNotification(`Analyzing profile for ${username}...`, 'info');
          ...
          const isDuplicate = await this.checkDuplicate(username);
          if (isDuplicate) {
          ...
          const result = await this.sendToServer(username);
  ...
  // Enhanced server communication with username
  async sendToServer(username) {
      const maxRetries = 3;
      let retryCount = 0;

      while (retryCount < maxRetries) {
          try {
              const response = await fetch('https://linkedinscrap-e4dpdhcuc7fgd7fk.eastasia-01.azurewebsites.net/api/profiles', {
                  method: 'POST',
                  headers: {
...
// Enhanced message listener - FIXED TO RECEIVE USERNAME FROM POPUP
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'scrapeProfile') {
      ...
      let username = request.username || localStorage.getItem('linkedin_scraper_username');
  ...
  if (request.action === 'setUsername') {
      // Store username when received from popup
...
if (typeof window !== 'undefined') {
  ...
  console.log('LinkedIn Assistant Enhanced with Authentication loaded');
}
